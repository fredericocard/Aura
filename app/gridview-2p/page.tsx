...
function CellInner({ player, defeated = false, defeatTrigger = 0 }: { player: any; defeated?: boolean; defeatTrigger?: string | number }) {
  const { keyframesNode, lifeAnimationStyle, fadeStyle, pulseAndWispsNode } = useDefeatAnimation(defeated, defeatTrigger, { pulseSize: 200, color: DARK.ink, smokeColor: DARK.ink3 });
...
function PageContent() {
  ...
  useEffect(() => {
    ...
    updateCurrentPage(gameId, uid, 'gridview-2p');
  ...
  // ── Check if either player accepted review (hides revive button) ──
  useEffect(() => {
