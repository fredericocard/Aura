...
import { concedeGame, updateLifeBySeat, updatePoisonBySeat, updateExperienceBySeat, updateEnergyBySeat, updateCommanderDamageBySeat, updateCurrentPage, getOpponentCurrentPage } from '@/lib/game-triggers';
...
function CellInner({ player, lifeSize = 64, defeated = false, defeatTrigger = 0 }: { player: any; lifeSize?: number; defeated?: boolean; defeatTrigger?: string | number }) {
  const { keyframesNode, lifeAnimationStyle, fadeStyle, pulseAndWispsNode } = useDefeatAnimation(defeated, defeatTrigger, { pulseSize: Math.round(lifeSize * 1.6) });
  ...
  return (
    <div style={{ position:'absolute', inset:0, borderRadius:'20px', overflow:'hidden' }}>
      ...
      <div style={{ position:'absolute', top:10, left:12, right:12,
        display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:8, ...fadeStyle }}>
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column',
        ...
        <div style={{ fontFamily:'var(--font-display)', fontWeight:400,
          ...
          textShadow: defeated ? undefined : '0 0 30px rgba(226,184,88,0.15), 0 1px 0 rgba(10,6,4,0.6)',
          ...lifeAnimationStyle,
        }}>{player.life}</div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <CellInner player={player} defeated={isDefeated} defeatTrigger={defeatTrigger}/>
      {isDefeated && (
        ...
        {isDefeated && (
          ...
          <div style={{
            ...
            <div style={{
              ...
              fontVariantNumeric:'tabular-nums',
              textShadow: isDefeated ? undefined : '0 0 30px rgba(226,184,88,0.15), 0 1px 0 rgba(10,6,4,0.6)',
              ...lifeAnimationStyle,
            }}>{life}</div>
...
function NormalCell({ player, onTapLeft, onTapRight, onRevive, lifeSize = 100, onHoldLeftStart, onHoldRightStart, onHoldEnd }: { player: any; onTapLeft: () => void; onTapRight: () => void; onRevive?: () => void; lifeSize?: number; onHoldLeftStart?: () => void; onHoldRightStart?: () => void; onHoldEnd?: () => void }) {
  ...
  return (
    <div style={{
      ...
      <CellInner player={player} lifeSize={lifeSize} defeated={isDefeated} defeatTrigger={defeatTrigger}/>
      {isDefeated && (
        <DefeatedButtonsLayer
          reviveColor={DARK.forest}
          reviveTextColor={DARK === LIGHT_THEME ? '#FAF5EA' : DARK.ink}
...
function PageContent() {
  ...
  useEffect(() => {
    ...
    updateCurrentPage(gameId, uid, 'gridview-3p');
  ...
  useEffect(() => {
    ...
    if (allDead) {
      ...
      // re-fires next time everyone dies again. Skip if summoning is active.
      ...
      {showVictory && (
        <VictoryPopup
          ...
          onRevive={async () => {
            ...
            const oppUid = lastDeadOpponent != null ? playerUserIds[lastDeadOpponent] : null;
            if (oppUid && gameId) {
      ...
      {showEliminatedGV && (
        <EliminatedPopup
          ...
          onRevive={async () => {
            ...
            const aliveOppEntry = Object.entries(playerUserIds).find(([k, v]) => Number(k) !== mySeat && v !== uid);
            const oppUid = aliveOppEntry ? aliveOppEntry[1] : null;
            if (oppUid && gameId) {
              const oppPage = await getOpponentCurrentPage(gameId, oppUid);
              if (oppPage && oppPage !== 'review') {
                setSummoningRevive(false);
