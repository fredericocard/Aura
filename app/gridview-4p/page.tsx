...
function CellInner({ player, defeated = false, defeatTrigger = 0 }: { player: any; defeated?: boolean; defeatTrigger?: string | number }) {
  const { keyframesNode, lifeAnimationStyle, fadeStyle, pulseAndWispsNode } = useDefeatAnimation(defeated, defeatTrigger, { pulseSize: 110, color: DARK.ink, smokeColor: DARK.ink3 });
...
function PageContent() {
  ...
  useEffect(() => {
    ...
    async function loadGame() {
      ...
      // Fetch current user's deck from pod_members (for seat picker art)
      if (myUserId && podId) {
