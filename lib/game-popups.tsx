...
// ─── Abandon Victory Popup (opponent left — you win, must review) ───────────
// Same crown layout as VictoryPopup but only "Go to Review". No revive, no
// dismiss, no backdrop tap. Browser back is blocked so the user can't escape.
export function AbandonVictoryPopup({
  onReview, theme,
}: {
  onReview: () => void;
  theme: PopupTheme;
}) {
  const visible = useDelayedEntrance(600);

  // Block browser back — push a state and re-push on every popstate
  useEffect(() => {
    const tag = "abandon-victory-block";
    if (!window.history.state?._tag || window.history.state._tag !== tag) {
      window.history.pushState({ _tag: tag }, "");
    }
    const onPop = () => {
      window.history.pushState({ _tag: tag }, "");
    };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, display: "flex", flexDirection: "column", fontFamily: "var(--font-ui)", pointerEvents: visible ? "auto" : "none" }}>
      ...
      <div style={{
        ...
        <TornEdge bgCard={theme.bgCard} />
        <div style={{ position: "relative", background: theme.bgCard, padding: "8px 22px 32px" }}>
          {/* Crown + heading — same as VictoryPopup */}
          <div style={{ textAlign: "center", marginTop: 6, marginBottom: 18 }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 10 }}>
              <svg width={32} height={32} viewBox="0 0 64 64" aria-hidden="true">
                <defs>
                  <linearGradient id="av-crown-grad" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stopColor="#E2B858" />
                    <stop offset="100%" stopColor="#C99B2F" />
                  </linearGradient>
                </defs>
                <path d="M10 48 L16 22 L24 36 L32 16 L40 36 L48 22 L54 48 Z" fill="url(#av-crown-grad)" stroke="#8C5A28" strokeWidth="1.5" strokeLinejoin="round" />
                <rect x="10" y="48" width="44" height="6" rx="1" fill="#C99B2F" stroke="#8C5A28" strokeWidth="1.5" />
                <circle cx="16" cy="22" r="2.5" fill="#F0E8D8" />
                <circle cx="32" cy="16" r="2.8" fill="#F0E8D8" />
                <circle cx="48" cy="22" r="2.5" fill="#F0E8D8" />
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: theme.ink3, lineHeight: 1.4 }}>
              Your opponent left the game. Head to review to wrap up and rate the match.
            </div>
          </div>

          {/* Single button — no alternative */}
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button onClick={onReview} style={{
              width: "100%", cursor: "pointer",
              background: theme.forest, color: theme.parchment,
